...
class PassTheBomb extends Rooms.RoomGame {
	constructor(room, seconds) {
		...
		this.room.add('|uhtml|bomb' + this.room.bombCount + this.round + '|<div class="broadcast-blue"><center>A game of <strong>Pass the Bomb</strong> has been started!<br>' +
			'The game will begin in <strong>' + seconds + '</strong> seconds!<br>' +
			'<button name = "send" value = "/ptb join">Join!</button></center></div>'
		);
		this.timer = setTimeout(() => {
			if (this.players.size < 4) {
				this.room.add('|uhtmlchange|bomb' + this.room.bombCount + this.round + '|<div class = "broadcast-blue"><center>This game of Pass the Bomb has been ended due to the lack of players.</center></div>').update();
				return this.end();
			}
			this.nextRound();
		}, seconds * 1000);
	...
	updateJoins() {
		let msg = 'bomb' + this.room.bombCount + this.round + '|<div class="broadcast-blue"><center>A game of <strong>Pass the Bomb</strong> has been started!<br>' +
	...
	getMsg() {
		let msg = 'bomb' + this.room.bombCount + this.round + '|<div class="broadcast-blue"><center><strong>Round ' + this.round + '</strong><br>' +
	...
	getWinner() {
		...
		let prize = 2;
		let msg = '|html|<div class = "broadcast-green"><center>Congratulations to ' + WL.nameColor(winner, true) + ' for winning the game of pass the bomb!</center>';
		this.room.add(msg).update();
		if (this.room.isOfficial) {
		    Economy.writeMoney(winner, prize);

...
exports.commands = {
	wheel: {
		create: function (target, room, user) {
			if (room.id !== 'lobby') return this.errorReply("This game may not be played outside of a casino.");
			...
			Economy.readMoney(user.userid, money => {
				...
				room.add('|raw|<b><font color="orange">' + WL.nameColor(user, true) + ' is hosting the wheel.</font></b>');
				room.update();
		...
		join: function (target, room, user) {
			...
			Economy.readMoney(user.userid, money => {
				...
				Economy.writeMoney(user.userid, (room.wheel.wheel[room.wheel.wheel.length - 1] * room.wheel.wheelMulti) * -1, function () {
					room.add('|raw|<b><font color="orange">' + WL.nameColor(user, true) + ' has spun the wheel.</font></b>');
			...
			delete room.wheel;
			room.add('|raw|<font color="orange">The WOM was ended by <b>' + WL.nameColor(user, true) + '</b>.</font>'); //html needed
		},

...
function leaguePM(message, league) {
	...
	for (let u in leagues[leagueid].users) {
		...
		Users(leagues[leagueid].users[u]).send("|pm|~Guild Master|~|/raw " + message);
	}
}
...
WL.getLeagueRank = getLeagueRank;
...
function leagueTourPoints(winner, runnerup, tourSize, room) {
	...
	if (winnerLeague && winnerPoints > 0) {
		...
		room.addRaw("<b>" + WL.nameColor(winner, true) + " has won " + winnerPoints + (winnerPoints === 1 ? " point " : " points ") + " for " + Chat.escapeHTML(leagues[winnerLeague].name) + "</b>.");
	}
	if (secondLeague && secondPoints > 0) {
		...
		room.addRaw("<b>" + WL.nameColor(runnerup, true) + " has won " + secondPoints + (secondPoints === 1 ? " point " : " points ") + " for " + Chat.escapeHTML(leagues[secondLeague].name) + "</b>.");
	}
}
WL.leagueTourPoints = leagueTourPoints;
...
function isLvLBattle(p1, p2, id, status, types, score) {
	...
	if (Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenger && Rooms.global.LvL[leagueId].challenger === targetLeagueid || Rooms.global.LvL[leagueId] && Rooms.global.LvL[leagueId].challenging && Rooms.global.LvL[leagueId].challenging === targetLeagueid) {
		...
		if (status === 'start') {
			...
			lvlDisplay(room);
			room.add('|raw|<a href="/' + id + '">The Guid vs Guild battle between ' + WL.nameColor(p1, true) + ' (' + Chat.escapeHTML(leagues[leagueId].name) + ') and ' +
				WL.nameColor(p2, true) + ' (' + Chat.escapeHTML(leagues[targetLeagueid].name) + ') has begun.</a>');
			...
			if (room.lvl.mode === "normal") {
				...
				if (room.lvl.status[room.lvl.statusNumber]) {
					...
					room.add('|raw|' +
						'<div class="infobox">Congratulations ' + Chat.escapeHTML(winner) + '. You have won the Guild vs Guild! You have been awarded ' +
						points + ' Guild Points.</div>'
					);
					room.update();
					leagues[toId(winner)].points += points;
					save();
					logPointsUser("GvG", toId(winner), points, "Won Guild vs Guild against " + loser + ".");
			...
			lvlDisplay(room);
			room.add("|raw|The Guild vs Guild battle between " + WL.nameColor(p1, true) + " and " + WL.nameColor(p2, true) + " ended with a tie. They have to have a rematch.");
			room.update();
		}
	}
}
WL.isLvLBattle = isLvLBattle;

function lvlDisplay(room) {
	let output = '';
	output += '<center><font size="6">Guild vs Guild</font><br />';
	output += '<font color="grey"><small>(experimental - report any bugs to Prince Sky)</small></font>';
	...
	if (room.lvl.leagues[0].players.length === room.lvl.size && room.lvl.leagues[1].players.length === room.lvl.size && !room.lvl.started) {
		...
		if (notOnline.length > 0) {
			...
			room.add("The following players have been removed from the Guild vs Guild due to not being online: " + notOnline.join(', '));
		} else {
			room.lvl.started = true;
			Dex.shuffle(room.lvl.leagues[0].players);
			Dex.shuffle(room.lvl.leagues[1].players);
			room.add("The Guild vs Guild has started!");
			room.lvl.status[0] = 2;
		}
	}
	...
	room.add('|uhtmlchange|GvG-' + room.lvl.lvlId + '|');
	room.add('|uhtml|RvR-' + room.lvl.lvlId + '|<div class="infobox">' + output + '</div>');
	room.update();
}
WL.lvlDisplay = lvlDisplay;
...
exports.commands = {
	...
	guilds: {
		create: function (target, room, user) {
			...
			if (!target) return this.errorReply("Usage: /guild create [region name], [user]");
		...
		invite: function (target, room, user) {
			if (!target) return this.errorReply("Usage: /guild invite [user] - Invites a user to your guild.");
			...
			leaguePM(WL.nameColor(user.name, true) + " has invited " + WL.nameColor(targetUser.name, true) + " to join the guild.", leagueid);
			let message = "/html has invited you to join the guild " + Chat.escapeHTML(leagues[leagueid].name) + ". <br />" +
		...
		accept: function (target, room, user) {
			if (!target) return this.errorReply("Usage: /guild accept [guild]");
			...
			leaguePM(WL.nameColor(user.name, true) + " has accepted their invitation to join the guild.", leagueid);

			user.popup("You've accepted the invitation to join " + leagues[leagueid].name + ".");
		},

		decline: function (target, room, user) {
			if (!target) return this.errorReply("Usage: /guild decline [guild]");
			let leagueid = toId(target);
			if (!leagues[leagueid]) return this.errorReply("That guild does not exist.");
			if (!leagues[leagueid].pendingInvites.includes(user.userid)) return this.errorReply("You don't have a pending invitation to this guild.");
		...
		kick: function (target, room, user) {
			if (!target) return this.errorReply("Usage: /guild kick [user] - Kicks a user from your guild.");
			...
			leaguePM(WL.nameColor(user.name, true) + " has kicked " + WL.nameColor(target, true) + " from the region.", leagueid);

			if (Users(target) && Users(target).connected) Users(target).send("|popup||html|" + WL.nameColor(user.name, true) + " has kicked you from the guild. " + Chat.escapeHTML(leagues[leagueid].name) + ".");
		...
		leave: function (target, room, user) {
			...
			leaguePM(WL.nameColor(user.name, true) + " has left the guild.", leagueid);
		...
		pm: function (target, room, user) {
			...
			leaguePM("Guild announcement from " + WL.nameColor(user.name, true) + ":<br />" + Chat.escapeHTML(target), leagueid);
		},
		...
		members: function (target, room, user) {
			...
			for (let rank in sortedRanks) {
				...
				for (let u in leagues[target].ranks[curRank].users) {
					...
					users.push(WL.nameColor(curUser, (Users(curUser) && Users(curUser).connected)));
				}
				output += users.join(',');
				output += "\n\n";
			}
		...
		rank: {
			...
			give: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has set " + WL.nameColor(targetUser.name, true) + "'s rank to " + Chat.escapeHTML(rank), leagueid);
				targetUser.send("|popup||html|" + WL.nameColor(user.name, true) + " has set your guild rank in " + Chat.escapeHTML(leagues[leagueid].name) + " to " +
			...
			take: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has removed the rank " + Chat.escapeHTML(rank) + " from " + WL.nameColor(targetUser, true), leagueid);
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + WL.nameColor(user.name, true) + " has removed you from the guild rank " + Chat.escapeHTML(rank) + " in " +
			...
			create: function (target, room, user) {
				if (!target) return this.errorReply("Usage: /gud rank create [rank title], [sortby (a number)], [permissions seperated by comma] - See '/guild rank permissions' to learn valid permissions.");
				let targets = target.split(',');
				for (let u in targets) targets[u] = targets[u].trim();
				...
				leaguePM(WL.nameColor(user.name, true) + " has added the rank '" + Chat.escapeHTML(rank) + "'.", leagueid);
			...
			delete: function (target, room, user) {
				...
				leagueLog(WL.nameColor(user.name, true) + " has deleted the rank '" + Chat.escapeHTML(rank) + "'.", leagueid);
			...
			help: function (target, room, user) {
				if (!this.runBroadcast()) return;
				this.sendReply("|raw|<div class=\"infobox infobox-limited\">" +
					"/guild rank create [rank title], [sortby (a number)], [permissions seperated by comma] - See <button style=\"background: none; border: none; color: blue\" name=\"send\" value=\"/guild rank permissions\"><u>/region rank permissions</u></button> for a list of valid permissions.<br />" +
					"/guild rank delete [rank title] - Deletes a guild rank. You have to remove the rank from members before deleting it.<br />" +
		...
		registerteam: function (target, room, user) {
			...
			getTeam(pastebin, (err, team) => {
				...
				leaguePM(WL.nameColor(user.name, true) + " has registered their team to become a challenger.", toId(league));
				this.sendReply("Your team has been registered. You may now challenge guild members with the '/guild challenge' command.");
				save();
			});
		},
		...
		challenge: function (target, room, user, connection) {
			...
			user.prepBattle(Dex.getFormat('ubers').id, 'challenge', connection).then(result => {
				if (result) {
					...
					targetUser.send('|pm|' + user.getIdentity() + '|~|/raw <div class="infobox">' + WL.nameColor(user.name, true) + ' is challenging your guild.</div>');
					user.send('|pm|' + targetUser.getIdentity() + '|~|/raw <div class="infobox">You are challenging ' + WL.nameColor(targetUser.userid, true) + '\'s guild.');
					leaguePM(WL.nameColor(user.name, true) + " has challenged " + WL.nameColor(targetUser.name, true) + ".");
				}
			});
		},
		...
		badges: {
			add: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has added the badge '" + Chat.escapeHTML(badgeName) + "'.", leagueid);
			...
			edit: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has edited the badge '" + Chat.escapeHTML(badgeName) + "'.", leagueid);
			...
			delete: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has deleted the badge '" + Chat.escapeHTML(badgeName) + "'.", leagueid);
			...
			give: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has given the badge '" + Chat.escapeHTML(badgeName) + "' to " + WL.nameColor(targetUser, true) + ".", leagueid);
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + WL.nameColor(user.name, true) + " has given you the region badge " + Chat.escapeHTML(badgeName) + " in " + Chat.escapeHTML(leagues[leagueid].name) + ".");
				}
			...
			take: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has taken the badge '" + Chat.escapeHTML(badgeName) + "' from " + WL.nameColor(targetUser, true) + ".", leagueid);
				if (Users(targetUser) && Users(targetUser).connected) {
					Users(targetUser).send("|popup||html|" + WL.nameColor(user.name, true) + " has taken the region badge " + Chat.escapeHTML(badgeName) + " from you in " + Chat.escapeHTML(leagues[leagueid].name) + ".");
				}
			...
			view: function (target, room, user) {
				...
				if (!this.runBroadcast()) return;
				let badges = WL.getBadges(target);
				if (badges.length < 1) return this.sendReplyBox(WL.nameColor(target, true) + " has no region badges.");
				let output = WL.nameColor(target, true) + "'s region badges:<br /><br />";
				for (let u in badges) {
		...
		gvg: {
			accept: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + ' has accepted the Guild vs Gud challenge against ' + Chat.escapeHTML(leagues[targetLeagueid].name), leagueId);
				leaguePM(WL.nameColor(user.name, true) + ' (' + leagues[leagueId].name + ') has accepted the Guild vs Guild challenge against your Guild.', targetLeagueid);
			...
			deny: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + ' has declined the Guild vs Guild challenge against ' + Chat.escapeHTML(leagues[targetLeagueid].name), leagueId);
				leaguePM(WL.nameColor(user.name, true) + ' (' + leagues[leagueId].name + ') has declined the Guild vs Guild challenge against your guild.', targetLeagueid);
				...
				if (!hasPermission(user.userid, 'lvl')) return this.errorReply("You don't have permission to end Guild vs Guild.");
				...
				targetRoom.add('|uhtmlchange|gvg-' + targetRoom.lvl.lvlId + '|');
				targetRoom.add('|uhtml|gvg-' + targetRoom.lvl.lvlId + '|(The Guild vs Guild has been forcibly ended by ' + Chat.escapeHTML(user.name) + ' (' + Chat.escapeHTML(leagues[leagueId].name) + '))');

				leaguePM(WL.nameColor(user.name, true) + ' has forcibly ended the Guild vs Guild with ' + Chat.escapeHTML(leagues[targetLeagueid].name) + '.', leagueId);
			...
			invite: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has invited " + WL.nameColor(targetUser.name, true) + " to join the Guild vs Guild against " + Chat.escapeHTML(leagues[targetLeague.id].name), leagueId);
				targetUser.send("|popup||modal||html|" + WL.nameColor(user.name, true) + " has invited you to join the Guild vs Guild against " + Chat.escapeHTML(leagues[targetLeague.id].name) +
			...
			join: function (target, room, user) {
				...
				if (league.players.length >= room.lvl.size) return this.errorReply("Your guild team is already full.");
			...
			leave: function (target, room, user) {
				if (!room.lvl) return this.errorReply("There's no Guid vs Guild in this room.");
				if (!getLeague(user.userid)) return this.errorReply("You're not in guild.");
				if (!room.lvl.accepted) return this.errorReply("This Guild vs Guild hasn't been accepted yet.");

				let leagueId = toId(getLeague(user.userid));
				if (room.lvl.leagues[0].id !== leagueId && room.lvl.leagues[0].id !== leagueId) return this.errorReply("Your guild is not apart of this Guid vs Guid.");

				let league = room.lvl.leagues[0];

				if (room.lvl.leagues[1].id === leagueId) league = room.lvl.leagues[1];
				if (!league.players.includes(user.userid)) return this.errorReply("You haven't joined this Guild vs Guild.");
				if (room.lvl.started) return this.errorReply("You can't leave a Guid vs Guild after it starts.");
			...
			challenge: function (target, room, user) {
				...
				leaguePM(
					WL.nameColor(user.name, true) + ' (' + Chat.escapeHTML(getLeague(user.userid)) + ') has challenged your guild to a Guild vs Guild (' +
					size + 'v' + size + ') in' +
					' <button name="joinRoom" value="' + room.id + '">' + Chat.escapeHTML(room.title) + '</button>.<br />' +
					'<button name="send" value="/guild gvg accept">Accept</button> | <button name="send" value="/guild gvg deny">Decline</button>', targetLeagueid
				);
				leaguePM(
					WL.nameColor(user.name, true) + ' has challenged ' + Chat.escapeHTML(leagues[targetLeagueid].name) + ' to a Guild vs Guild (' +
					size + 'v' + size + ') in <button name="joinRoom" value="' + room.id + '">' + Chat.escapeHTML(room.title) + '</button>'
				);
				room.add('|uhtml|lvl-' + lvlId + '|' +
					'<div class="infobox"><center>' + WL.nameColor(user.name, true) + ' has challenged ' + Chat.escapeHTML(leagues[targetLeagueid].name) +
			...
			help: function (target, room, user) {
				if (!this.runBroadcast()) return;
				this.sendReply("|raw|<div class=\"infobox infobox-limited\">" +
					"Guild vs GuildCommands:<br />" +
					"/guild gvg challenge [guild], [mode (normal or quick)], [size (must be odd number)] - Challenges a guild to a Guild vs Guild in the current room.<br />" +
		...
		points: {
			give: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has given your guild " + amount + (amount === 1 ? " point." : " points."), league);
			},
			...
			take: function (target, room, user) {
				...
				leaguePM(WL.nameColor(user.name, true) + " has taken " + amount + (amount === 1 ? " point " : " points ") + " from your guild.", league);
			},
			...
			reset: function (target, room, user) {
				...
				WL.messageSeniorStaff("/html " + WL.nameColor(user.name, true) + " has reset all guild points.");
				Rooms('staff').add("|raw|" + WL.nameColor(user.name, true) + " has reset all guild points.").update();
			...
			log: function (target, room, user, connection, cmd) {
				...
				database.all("SELECT * FROM points WHERE " + (cmd === 'userlog' ? "userid=$userid " : "league=$leagueid ") + "ORDER BY date DESC LIMIT 500", searchObj, (err, rows) => {
					...
					for (let u in rows) {
						output += '<tr>';
						output += '<td>' + WL.nameColor(rows[u].userid, (Users(rows[u].userid) && Users(rows[u].userid).connected)) + '</td>';
			...
			help: function (target, room, user) {
				...
				this.sendReply("|raw|<div class=\"infobox infobox-limited\">" +
					...
					"/guild points take [region], [amount] - Takes points from guild<br />" +
		...
		help: function (target, room, user) {
			...
			return this.sendReply(
				...
				"/guild pm [message] - Mass PM's a message to all online region members<br />" +
				...
				"/guild resetteam [user] - Resets a users registered team so they can register again. Requires permission level 4 or higher. (Default: Masters)<br /><br />" +
				...
				"/guild rank delete [rank name] - Deletes a region rank. Note: you can't delete a rank if any users currently have the rank.<br /><br />" +
				/*"Region Badge Commands: <br />" +
				...
				"/gvg challenge [guild], [mode (normal or quick)], [size (must be odd number)] - Challenges a guild to a Guild vs Guilf in the current room.<br />" +
				"(Quick mode lets players battle each other at the same time, normal mode limits it to one battle at a time.)<br />" +
				"/gvg accept - Accepts a challenge from a guild.<br />" +
				"/gvg deny - Denies a challenge from a guild..<br />" +
				"/gvg invite [user] - Invites a guild member to join the guild vs guild.<br />" +
	...
	gvg: function (target, room, user) {
		return this.parse('/guild gvg ' + target);
	},
	guildhelp: function (target, room, user) {

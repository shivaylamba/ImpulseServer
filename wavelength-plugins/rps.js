...
class RPSGame {
	...
	parseWin(winner, loser, inactivity) {
		...
		if (this.gameType === "bucks") {
			...
			//do rank change
			let winnerPoints = Sb("rpsrank").get(winner.userid, 1000);
			let loserPoints = Sb("rpsrank").get(loser.userid, 1000);
			...
			Sb("rpsrank").set(winner.userid, winnerFinalPoints);
			...
			Sb("rpsrank").set(loser.userid, loserFinalPoints);
...
exports.commands = {
	rps: {
		...
		rank: function (target, room, user) {
			...
			let userRank = Sb("rpsrank").get(toId(target), 1000);
		...
		ladder: function (target, room, user) {
			...
			let index = 1;
			let table = Object.keys(Sb("rpsrank").object()).sort(function (a, b) {
				if (Sb("rpsrank").get(a, 1000) > Sb("rpsrank").get(b, 1000)) return -1;
				return 1;
			}).slice(0, 100).map(function (u) {
				return '<tr><td>&nbsp;' + index++ + '&nbsp;</td><td>&nbsp;' + u + '&nbsp;</td><td>&nbsp;' + Sb("rpsrank").get(u, 1000) + "&nbsp;</td></tr>";
			}).join("");

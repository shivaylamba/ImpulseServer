...
exports.commands = {
	...
	leagueshop: {
		enable: function (target, room, user) {
			...
			if (Sb('roomshop').has(room.id)) return this.errorReply('Roomshop is already enabled here.');
		...
		disable: function (target, room, user) {
			if (!this.can('hotpatch')) return false;
			if (!Sb('roomshop').has(room.id)) return this.errorReply('Roomshop is not enabled here.');

			Sb('roomshop').delete(room.id);
		...
		bank: {
			set: function (target, room, user) {
				...
				if (!Sb('roomshop').has(room.id)) return this.errorReply('Roomshop is not enabled here.');
				if (Sb('roomshop').has([room.id, 'Bank'])) return this.errorReply(room.id + ' already has a bank.');
			...
			change: function (target, room, user) {
				if (!this.can('declare', null, room)) return false;
				if (!Sb('roomshop').has(room.id)) return this.errorReply('Roomshop is not enabled here.');
				if (!Sb('roomshop').has([room.id, 'Bank'])) return this.errorReply(room.id + ' does not have a bank set.');
			...
			'': function (target, room, user) {
				...
				let bank = Sb('roomshop').get([room.id, 'Bank']);
		...
		add: function (target, room, user) {
			...
			if (Sb('roomshop').has([room.id, itemID])) return this.errorReply(itemID + ' is already present in roomshop.');

			let keys = Object.keys(Sb('roomshop').object()[room.id]);
		...
		remove: function (target, room, user) {
			...
			if (!Sb('roomshop').has([room.id, itemID]) || itemID === 'bank') return this.errorReply(itemID + ' is not present in roomshop.');
			Sb('roomshop').delete([room.id, itemID]);
		...
		buy: function (target, room, user) {
			...
			if (!Sb('roomshop').has([room.id, itemID]) || itemID === 'bank') return this.errorReply(itemID + ' is not present in roomshop.');
			if (!Sb('roomshop').has([room.id, 'Bank'])) return this.errorReply(room.id + ' does not have a bank set.');
			if (Sb('roomshop').get([room.id, 'Bank']) === user.userid) return this.errorReply('Bank cannot purchase from the roomshop.');

			let usersMoney = Db.currency.get(user.userid, 0);
			let bank = Sb('roomshop').get([room.id, 'Bank']);
			let banksMoney = Db.currency.get(bank, 0);
			let cost = Sb('roomshop').get([room.id, itemID, 'Price']);
		...
		'': function (target, room, user) {
			...
			function display(keys) {
				...
				for (let i = 0; i < keys.length; i++) {
					let item = Sb('roomshop').object()[room.id][keys[i]];

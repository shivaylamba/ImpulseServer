...
function display(result, user, slotOne, slotTwo, slotThree) {
	...
	if (!result) {
		display += 'Aww... bad luck, <b>' + WL.nameColor(user.name, true) + '. Better luck next time!</font>';
	}
	if (result) {
		display += 'Congratulations, ' + WL.nameColor(user.name, true) + '. You have won ' + slots[slotOne] + ' bucks!!</font>';
	}
	return display + '</div>';
}
	...
	slots: {
		...
		spin: function (target, room, user) {
			...
			if (chancePercentage >= chancesGenerated) {
				Db.currency.set(user.userid, (amount + slots[result]));
			...
			Db.currency.set(user.userid, (amount - 3));
